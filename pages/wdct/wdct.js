...
Page({
  ...
  delCt() {
    let nowTestTm = this.data.nowTestTm
    app.$api.post('/qb/sub/wrong/remove', {
      ids: nowTestTm.wrongId
    })
  },
  // 多选提交答案
  submitTm() {
  ...
  async getTestTm() {
    ...
    await app.$api.post('/qb/sub/wrong/list').then(res => {
      Toast.clear();
      let allTmList = res.data.map(val => {
        val.qbSubject.wrongId = val.id
        val.qbSubject.collectionId = val.qbSubject.collectId
